...
List RothC(DataFrame soc_data, double max_tsmd, double iom, double clay, double sat, bool use_atsmd = 0){
	...
	unsigned int n = time.size();
    NumericVector tsoc(n), b(n);
	NumericVector dec_dpm(n), dec_rpm(n), dec_doc(n), dec_bio(n), dec_hum(n), dec_tot(n);
	NumericVector min_dpm(n), min_rpm(n), min_bio(n), min_hum(n), min_tot(n), min_cum(n);
	NumericVector dpm_d13c(n), rpm_d13c(n), bio_d13c(n), hum_d13c(n), co2_d13c(n);
	// initialise soc pools d13C to bulk soc d13C
	dpm_d13c[0] = rpm_d13c[0] = bio_d13c[0] = hum_d13c[0] = co2_d13c[0] = soc_d13c[0]; 
	
	double k1 = 0.8333333;
	double k2 = 0.025;
	double k3 = 0.055;
	double k4 = 0.001666667;
	
	// partitioning of decomposition between bio, hum & co2
	double f_bio, f_hum, f_co2, x;
	x = 1.67 * (1.85 + 1.6 * std::exp(-0.0786 * clay));
	f_bio = 0.46 * (1 - (x/ (1 + x)));
	f_hum = 0.54 * (1 - (x/ (1 + x)));
	f_co2 = x / (1+x);
	
	for (unsigned int i = 0; i < n; i++) {
		if (i > 0) {  // first month pools & d13C already initialized. 
			// pools
			dpm[i] = dpm[i-1] - dec_dpm[i-1] + add_dpm[i-1];
			rpm[i] = rpm[i-1] - dec_rpm[i-1] + add_rpm[i-1];
			bio[i] = bio[i-1] - dec_bio[i-1] + add_bio[i-1] + dec_tot[i-1]*f_bio;
			hum[i] = hum[i-1] - dec_hum[i-1] + add_hum[i-1] + dec_tot[i-1]*f_hum;
			soc[i] = dpm[i] + rpm[i] + bio[i] + hum[i];
			
			//d13C
			dpm_d13c[i] = ((dpm[i-1] - dec_dpm[i-1])*dpm_d13c[i-1] + add_dpm[i-1]*add_d13c[i-1]) / (dpm[i-1]-dec_dpm[i-1]+add_dpm[i-1]);
			rpm_d13c[i] = ((rpm[i-1] - dec_rpm[i-1])*rpm_d13c[i-1] + add_rpm[i-1]*add_d13c[i-1]) / (rpm[i-1]-dec_rpm[i-1]+add_rpm[i-1]);
			bio_d13c[i] = ((bio[i-1] - dec_bio[i-1]*(1-f_bio)) * bio_d13c[i-1]
			  + f_bio*dec_dpm[i-1] * dpm_d13c[i-1]
			  + f_bio*dec_rpm[i-1] * rpm_d13c[i-1]
			  + f_bio*dec_hum[i-1] * hum_d13c[i-1])
			  / (bio[i-1] - dec_bio[i-1] + f_bio * dec_tot[i-1]);

			hum_d13c[i] = ((hum[i-1] - dec_hum[i-1]*(1-f_hum)) * hum_d13c[i-1]
			  + add_hum[i-1] * add_d13c[i-1]
			  + f_hum*dec_dpm[i-1] * dpm_d13c[i-1]
			  + f_hum*dec_rpm[i-1] * rpm_d13c[i-1]
			  + f_hum*dec_bio[i-1] * bio_d13c[i-1])
			  / (hum[i-1]-dec_hum[i-1]+add_hum[i-1]+f_hum*dec_tot[i-1]);
		}
		tsoc[i] = soc[i] + iom;
	...
	if (use_atsmd) {
      ...
      b[i] =  (h2o[i] > 0.556*sat) ? 1 : 0.2 + 0.8 * h2o[i] / (0.556*sat);
		...
		// Decomposition
		dec_dpm[i] = dpm[i] * (1 - std::exp(-k1 * a[i] * b[i] * c[i] ));
		dec_rpm[i] = rpm[i] * (1 - std::exp(-k2 * a[i] * b[i] * c[i] ));
		dec_bio[i] = bio[i] * (1 - std::exp(-k3 * a[i] * b[i] * c[i] ));
		dec_hum[i] = hum[i] * (1 - std::exp(-k4 * a[i] * b[i] * c[i] ));
		dec_tot[i] = dec_dpm[i] + dec_rpm[i] + dec_bio[i] + dec_hum[i];

		// Mineralization
		min_dpm[i] = dec_dpm[i] * f_co2;
		min_rpm[i] = dec_rpm[i] * f_co2;
		min_bio[i] = dec_bio[i] * f_co2;
		min_hum[i] = dec_hum[i] * f_co2;
		min_tot[i] = min_dpm[i] + min_rpm[i] + min_bio[i] + min_hum[i];
		min_cum[i] = min_tot[i] + (i==0 ? 0 : min_cum[i-1]);  
		
		//d13C of respired CO2 and total soil organic carbon
		co2_d13c[i] = (min_dpm[i]*dpm_d13c[i] + min_rpm[i]*rpm_d13c[i] + min_bio[i]*bio_d13c[i] + min_hum[i]*hum_d13c[i]) / min_tot[i];
		soc_d13c[i] = (dpm[i]*dpm_d13c[i] + rpm[i]*rpm_d13c[i] + bio[i]*bio_d13c[i] + hum[i]*hum_d13c[i]) / soc[i];
		}
	...
	output["decomp.hum"] = dec_hum;
	output["decomp.tot"] = dec_tot;
	output["mineralise.dpm"] = min_dpm;
	output["mineralise.rpm"] = min_rpm;
	output["mineralise.bio"] = min_bio;
	output["mineralise.hum"] = min_hum;
	output["mineralise.tot"] = min_tot;
	output["mineralise.cum"] = min_cum;
	output["dpm.d13c"] = dpm_d13c;
	output["rpm.d13c"] = rpm_d13c;
	output["bio.d13c"] = bio_d13c;
	output["hum.d13c"] = hum_d13c;
	output["soc.d13c"] = soc_d13c;
	output["co2.d13c"] = co2_d13c;
	return output;
}
